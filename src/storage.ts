import type { IList, GistConfigState, IGistEntry, IGistConfig, TGistRegistry } from "./types.ts";
...
export function loadGistConfigState(): GistConfigState {
  ...
  let s: { gistToken?: string; gistEntries?: IGistEntry[] } = {};
...
export function buildRegistry(config: IGistConfig): TGistRegistry {
  const all = getListsFromStorage();
  const registry: TGistRegistry = {};
